    ...
    try:
        ...
        for old_module, new_module in import_mapping.items():
            ...
            if re.search(pattern1, content):
                ...
                changes_made.append(
                    f"Updated: from {old_module} import -> from {new_module} import"
                )
            ...
            if re.search(pattern2, content):
                ...
                changes_made.append(
                    f"Updated: import {old_module} -> import {new_module}"
                )
        ...
        if content != original_content:
            # Create backup
            backup_path = (
                str(file_path) + f".backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
...
def main():
    ...
    if fixed_count > 0:
        ...
        print(
            '   python -c "import path_helper; from src.rtm.rtm_pipeline import main; main()"'
        )
