...
markdown_generator = import_module(
    utils_dir / "markdown_generator.py", "markdown_generator"
)
ci_cd_integration = import_module(
    utils_dir / "ci_cd_integration.py", "ci_cd_integration"
)
...
if not docx_converter:
    ...
    try:
        import rtm_pipeline
        import src.parsers.enhanced_word_to_md
        import src.parsers.pandoc_converter
        import src.parsers.try_word_to_md
        import src.parsers.exact_docx_to_md
    except ImportError:
...
def main():
    ...
    parser.add_argument(
        "--skip-enhance", action="store_true", help="Skip markdown enhancement"
    )
    parser.add_argument(
        "--skip-requirements", action="store_true", help="Skip requirement extraction"
    )
    parser.add_argument(
        "--skip-digital-twin", action="store_true", help="Skip digital twin creation"
    )
    parser.add_argument("--skip-rtm", action="store_true", help="Skip RTM generation")
    parser.add_argument(
        "--ci-checks", action="store_true", help="Run CI/CD integration checks"
    )
    ...
    if args.list_files:
        try:
            ...
            if not docx_files:
                print(
                    "No DOCX files found in the input directory or current directory."
                )
                return 1
            ...
            print(
                "Could not import required module. Please ensure run_conversion.py exists."
            )
            return 1
    ...
    try:
        ...
        if args.input_file and Path(args.input_file).suffix.lower() in [
            ".docx",
            ".doc",
        ]:
        ...
        elif args.input_file and Path(args.input_file).suffix.lower() in [
            ".md",
            ".markdown",
        ]:
            ...
            logger.info(
                f"Input is already markdown, skipping conversion: {args.input_file}"
            )
        else:
        ...
        if "rtm" in results and results["rtm"]:
            logger.info(
                f"- RTM Report: {results['rtm'].get('rtm_report', 'Not generated')}"
            )
