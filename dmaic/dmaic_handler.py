...
Ensure long-term sustainability of the implemented solutions."""
    ...
    def start_phase(self, phase: DMAICPhase):
        ...
        if not isinstance(phase, DMAICPhase):
            raise ValueError(f"Phase must be a DMAICPhase enum value, got {type(phase)}")
    ...
    def interact(self, user_input):
        ...
        conversation_history = self.project_data["phases"][current_phase]["conversation"]
    ...
    def complete_phase(self, outputs=None):
        ...
        self.project_data["phases"][current_phase]["completed_at"] = datetime.now().isoformat()
    ...
    def save_project(self, filepath=None):
        ...
        """
        if filepath is None:
            filename = f"{self.project_name.replace(' ', '_').lower()}_dmaic_project.json"
    ...
    def run_phase_with_openai(self, phase: DMAICPhase, openai_config: Optional[Dict[str, Any]] = None):
        """
