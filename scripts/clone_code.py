...
def check_git_installed():
    ...
    try:
        ...
        return False, result.stderr.strip() if result.stderr else "Git command failed without specific error."
    except FileNotFoundError:
        return False, "Git executable not found in PATH. Please ensure Git is installed and in your system's PATH."
...
def get_repo_list():
    ...
    if not repos_file.exists():
        ...
        try:
            ...
            print_status(
                "INFO", f"Created default repositories file: {repos_file}")
            return default_repos_data["repositories"]
        except IOError as e:
            print_status("ERROR", f"Failed to write default repositories file {repos_file}: {e}")
            return []
    ...
    try:
        ...
        print_status("ERROR", f"An unexpected error occurred while loading {repos_file}: {e}")
        return []
...
def clone_repository(repo_info, update=True):
    ...
    if not all([url, target_dir_str]):
        print_status("ERROR", "Repository info is missing URL or target_dir.", f"Details: {repo_info}")
        return False
        ...
        if update:
            ...
            try:
                ...
                if fetch_result.returncode != 0:
                    print_status(
                        "WARNING", "Failed to fetch updates.", fetch_result.stderr.strip())
                ...
                if current_branch != branch:
                    print_status(
                        "INFO", f"Switching from branch '{current_branch or 'detached HEAD'}' to '{branch}'")
                    ...
                    if checkout_result.returncode != 0:
                        print_status(
                            "WARNING", f"Failed to switch branch to '{branch}'.", checkout_result.stderr.strip())
                ...
                if pull_result.returncode == 0:
                    ...
                    if pull_result.stdout.strip() and pull_result.stdout.strip() != "Already up to date.":
                        print(f"       Changes: {pull_result.stdout.strip()}")
                    return True
                print_status(
                    "WARNING", f"Failed to pull changes for branch '{branch}'.", pull_result.stderr.strip())
                return False
            except Exception as e:
                print_status("ERROR", f"An unexpected error occurred during repository update: {e}")
                return False
            finally:
