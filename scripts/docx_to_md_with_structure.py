...
def convert_docx_to_md(input_file, output_file, structure_output=None, lua_filter=None):
    """
    ...
    try:
        ...
        # Add common options
        cmd.extend(['--extract-media', f"{output_dir}/media"])
        ...
        if structure_output:
            ...
            if extract_structure_from_md:
                ...
                if not extract_structure_from_md(output_file, structure_output):
            ...
            # Alternatively, if we have raw structure from Lua filter, process it
                ...
                raw_structure_file = "output/document_structure_raw.json"
                if os.path.exists(raw_structure_file):
                    ...
                    logger.warning(f"Raw structure file not found: {raw_structure_file}")
...
def generate_structure_txt(structure_data, output_file):
    """
    ...
    try:
        ...
        if dependencies:
            ...
            for source, targets in dep_by_source.items():
                ...
                source_desc = next((req['text'][:30] + "..." for req in requirements if req.get('id') == source), "")
                ...
                for i, (target, rel_type) in enumerate(targets):
                    if i == 0:
                        ...
                        target_desc = next((req['text'][:30] + "..." for req in requirements if req.get('id') == target), "")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Convert DOCX to Markdown with structure extraction")
    parser.add_argument("input", help="Input DOCX file")
    parser.add_argument("output", help="Output Markdown file")
    parser.add_argument("--structure", help="Output file for document structure", default="output/document_structure.txt")
    parser.add_argument("--lua-filter", help="Lua filter for structure extraction", default="config/structure_extraction.lua")
    ...
    lua_filter = args.lua_filter if os.path.exists(args.lua_filter) else None
    if not lua_filter:
        logger.warning(f"Lua filter not found: {args.lua_filter}")
    
    # Perform conversion
    success = convert_docx_to_md(args.input, args.output, structure_output, lua_filter)
...
REQ-050 The system depends on Python 3.7 or newer. [priority:high] [status:implemented]
</div>

<div class="requirement">
REQ-051 The system depends on Pandoc 2.0 or newer. [priority:high] [status:implemented]
</div>

<div class="requirement">
REQ-052 The system depends on the PyYAML package. [priority:high] [status:implemented]
</div>

<div class="requirement">
REQ-053 The system depends on the python-docx package. [priority:high] [status:implemented]
</div>

## 7.4 References to TraceReq Relationships

The requirements trace to other requirements as follows:

| ID | Depends On | Refines | Related To |
|----|------------|---------|------------|
| REQ-005 | REQ-001 | | REQ-045 |
| REQ-007 | | REQ-022 | |
| REQ-008 | REQ-007 | | |
| REQ-026 | REQ-022, REQ-023, REQ-024 | | |
| REQ-031 | REQ-006 | | |
