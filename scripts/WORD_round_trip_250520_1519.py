...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.warning("Specialized converter not found, falling back to standard converter")
            from src.modules.pandoc_integration import convert_document as convert_docx_to_md
        
        # Structure output path
        structure_output = args.structure_output
        if not structure_output and args.generate_structure:
            structure_output = os.path.join(output_dir, f"{input_path.stem}_structure.txt")
        ...
        if 'docx_to_md_with_structure' in sys.modules:
            ...
            if args.generate_structure and success:
                try:
                    ...
                    logger.warning("Structure generator module not available, skipping structure extraction")
