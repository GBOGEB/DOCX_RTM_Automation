...
def main():
    parser = argparse.ArgumentParser(description="Push RTM outputs to GitHub repository")
    ...
    parser.add_argument("--include-rtm", action="store_true", default=True, help="Include RTM files")
    parser.add_argument("--include-outline", action="store_true", default=True, help="Include outline files")
    parser.add_argument("--include-diagrams", action="store_true", default=True, help="Include diagram files")
    parser.add_argument("--include-all", action="store_true", help="Include all files in output directory")
    ...
    if not github_config.get("enabled", False):
        ...
        logger.error("To enable, set 'enabled: true' in the github section of your configuration")
        return 1
    ...
    if args.include_all:
        ...
        if args.include_rtm:
            rtm_files = []
            rtm_files.extend(list(output_dir.glob("requirements_traceability_matrix.*")))
