        ...
        if file_path.exists():
            ...
            if filename.endswith('.json'):
                try:
                    ...
                    print(f"   Requirements found: {len(metadata.get('requirements_found', []))}")
                    print(f"   Headings: {metadata.get('headings_count', 0)}")
                    print(f"   Lines: {metadata.get('lines_count', 0)}")
                    print(f"   Markdown processed: {metadata.get('markdown_processed', False)}")

                    if 'requirements_found' in metadata and metadata['requirements_found']:
                        print(f"   Requirements: {', '.join(metadata['requirements_found'])}")
                ...
                try:
                    ...
                    metadata = data.get('metadata', {})
                    print(f"   YAML metadata loaded successfully")
