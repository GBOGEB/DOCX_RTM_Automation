...
def main():
    ...
    parser.add_argument(
        "step", help="The pipeline step to test (word_to_md, extract_outline, etc.)"
    )
    parser.add_argument(
        "--input", help="Input file or directory (default: from config)"
    )
    parser.add_argument(
        "--output", help="Output file or directory (default: from config)"
    )
    parser.add_argument(
        "--config", help="Path to configuration file (default: config/paths.yaml)"
    )
    parser.add_argument(
    ...
    step_paths = {
        ...
        "extract_outline": os.path.join(
            PROJECT_ROOT, "src", "extractors", "extract_outline.py"
        ),
        "extract_rtm": os.path.join(
            PROJECT_ROOT, "src", "extractors", "extract_rtm.py"
        ),
        "md_to_json_yaml": os.path.join(
            PROJECT_ROOT, "src", "core", "md_to_json_yaml.py"
        ),
        "sync_outline_files": os.path.join(
            PROJECT_ROOT, "src", "utils", "sync_outline_files.py"
        ),
    }
    ...
    if (
        "--output-dir" not in " ".join(sys_args)
        and "--output" not in " ".join(sys_args)
        and "-o" not in " ".join(sys_args)
    ):
