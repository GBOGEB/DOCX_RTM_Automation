...
def _process_yaml_data_to_sections(data, current_level=1) -> list[dict]:
    ...
    if isinstance(data, list):
        ...
        if children:
            flat_sections.extend(_process_yaml_data_to_sections(children, current_level + 1))
...
def extract_hierarchical_structure(input_path: Path) -> dict | None:
    ...
    try:
        ...
        print(f"Could not parse as standard YAML ({ye}), attempting line-based extraction.")
