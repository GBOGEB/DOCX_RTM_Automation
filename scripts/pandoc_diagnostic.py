...
def test_lua_filter(filter_path, test_markdown):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False) as in_file:
...
def test_word_to_markdown_conversion(input_file=None):
    ...
    if not input_file:
        ...
        try:
            ...
            doc.add_paragraph('The system shall perform validation before saving data. [priority:high]')
            doc.add_heading('Section 2', 1)
            doc.add_paragraph('Another requirement: REQ-001 The system must handle errors gracefully.')
            ...
            result['error'] = "python-docx is not installed, cannot create test document"
            return result
        except Exception as e:
...
def run_diagnostics(args):
    ...
    lua_files_success = len(lua_files_result['found']) > 0 and len(lua_files_result['missing']) == 0
    results.add_test(
        name="lua_filters",
    ...
    if pandoc_result['installed'] and pandoc_result['lua_support'] and lua_files_result['found']:
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Pandoc and Lua Filter Diagnostic Tool")
    parser.add_argument("--json", action="store_true", help="Output results as JSON")
    parser.add_argument("--test-conversion", action="store_true", help="Test Word to Markdown conversion")
    ...
    if args.json:
        ...
        for test in summary['tests']:
            ...
            color = "\033[92m" if test['success'] else "\033[91m"  # Green or Red
        ...
        if summary['success']:
            print("\033[92mAll tests passed! Pandoc integration is working correctly.\033[0m")
        else:
            print("\033[91mSome tests failed. Please check the detailed results above.\033[0m")
