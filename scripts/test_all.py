...
def main():
    ...
    print(
        "This script will run a series of tests to ensure all components of the system are working correctly.\n"
    )
    ...
    dmaic_test = run_test("python examples/dmaic_example.py", "DMAIC Example Script")
    ...
    docx_test = run_test(
        "python utils/docx_converter.py test_doc.docx", "DOCX to Markdown Conversion"
    )
...
def test_function(a, b, c):
    ...
    refactor_test = run_test(
        "python refactor.py test_refactor.py --mode quick", "Code Refactoring"
    )
    ...
    if markdown_fix_test_passed:  # Check if the script execution itself passed
        ...
        try:
            ...
            if expected_fixed_content_normalized == fixed_content_normalized:
                print(
                    f"✓ Markdown file '{dummy_md_path}' content matches expected fixed state."
                )
            else:
                print(
                    f"✗ Markdown file '{dummy_md_path}' content not as expected after fix."
                )
                print(f"Expected:\n'''{expected_fixed_content_normalized}'''")
                print(f"Got:\n'''{fixed_content_normalized}'''")
                markdown_fix_test_passed = (
                    False  # Mark the test as failed if content doesn't match
                )
