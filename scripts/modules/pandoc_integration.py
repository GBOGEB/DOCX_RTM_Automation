...
def _find_lua_filter(filter_name: str = "rtm_filter.lua", config_dir: Optional[str] = None) -> Optional[Path]:
    ...
    logger.warning(f"Lua filter '{filter_name}' not found in search paths: {search_paths}")
    return None
...
def convert_document(
    ...
    cmd = ["pandoc", str(input_path), "-o", str(output_path), f"--from={from_format}", f"--to={to_format}"]
    ...
    if config:
        ...
        if lua_filter_config:
            ...
            if not lua_filter_path.exists():
                logger.warning(f"Lua filter specified in config not found: {lua_filter_config}")
                lua_filter_path = None
