...
def main():
    ...
    if not client:
        print("Error: Failed to initialize OpenAI client. Check API key and connectivity.")
        return
...
"""
    ...
    test_requirement = "REQ-002: The system should allow users to reset their passwords via email"
    print("\nAnalyzing requirement:")
    print(test_requirement)

    response, _ = rtm_agent(f"Analyze this requirement and suggest improvements: {test_requirement}")
    ...
    if 'output_dir' in paths:
        ...
        try:
            ...
            print(f"Error: Could not create or write to output directory {output_dir_path}. {e}")
    else:
        print("Warning: 'output_dir' not specified in project paths configuration. Cannot save analysis.")
