...
from agents.agent_common import (
    AgentMessage,
)  # Added for potential direct message sending
...
def run_example():
    ...
    if not output_dir or not os.path.isdir(
        os.path.dirname(output_dir)
    ):  # Check if parent of timestamped dir exists
        print(
            f"Error: Output directory path from PathsManager is invalid or its parent does not exist: {output_dir}"
        )
        # Attempt to create project_root/outputs as a fallback
        fallback_output_dir = os.path.join(
            project_root, "outputs", "copilot_example_logs"
        )
    ...
    copilot.register_with_orchestrator(
        None
    )  # Simulate registration for direct calls if orchestrator isn't used in example
    ...
    print(
        f"Repository analysis complete. Found {repo_analysis['file_count']} files "
    ...
    try:
        os.makedirs(
            os.path.dirname(example_json_path), exist_ok=True
    ...
    conversion_result = copilot.convert_file(
        example_json_path, example_yaml_path, ConversionType.JSON_TO_YAML
    )
    ...
    if generated_script_path and os.path.exists(generated_script_path):
        ...
        print(
            f"Bash script generation reported success, but file not found at: {generated_script_path}"
        )
    else:
        print(
            f"Bash script generation failed or path not returned. Target was: {script_path_target}"
        )
    ...
    if os.path.exists(code_path_target):
        ...
        print(
            f"Python code generation attempted to {code_path_target}, but file not found."
        )

    # 5. Extract requirements from a document (using the example.py file as source)
    print("\n5. Extracting requirements...")
    example_file_for_req_extraction = os.path.join(
        project_root, "examples", "dmaic_example.py"
    )
    if os.path.exists(example_file_for_req_extraction):
        req_output_path = os.path.join(output_dir, "extracted_requirements.json")
        print(
            f"Attempting requirements extraction from {example_file_for_req_extraction} to {output_dir}..."
        )
        copilot.extract_requirements_from_docs(
            paths_to_docs=[example_file_for_req_extraction], output_dir=output_dir
        )
        print(
            f"Requirements extraction attempted. Check logs and {output_dir} for results (e.g., {req_output_path})."
        )
    else:
        print(
            f"Example file for requirement extraction not found: {example_file_for_req_extraction}"
        )
    ...
    if os.path.exists(workflow_path_target):
        ...
        print(
            f"GitHub Actions workflow generation attempted to {workflow_path_target}, but file not found."
        )
    ...
    if os.path.exists(report_path_target):
        ...
        print(
            f"Analysis report generation attempted to {report_path_target}, but file not found."
        )
...
Some notes here.
    ...
    print(
        "Conceptual document parsing. Requires 'parse_document_command' handler in CopilotAgent."
    )
