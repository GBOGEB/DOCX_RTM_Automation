...
def fix_json_syntax_errors(content):
    ...
    for line in lines:
        ...
        if brace_count == 0 and current_object and any('{' in l for l in current_object):
...
def fix_vscode_settings():
    ...
    try:
        ...
        try:
            json.loads(content)
            print(f"   ✅ Settings fixed successfully!")
            ...
            print(f"   💡 Consider recreating VS Code settings")
            return False
...
def main():
    ...
    if any(".vscode/settings.json" in str(f[0]) for f in problematic_files):
        ...
        if not fix_vscode_settings():
            print(f"   💡 Creating minimal settings...")
    ...
    if failed_count == 0:
        ...
        print(f"🚀 Your RTM system's JSON ecosystem is healthy!")
    else:
        print(f"\n⚠️ Some files still need manual attention")
        print(f"💡 Check backup files for original content")
