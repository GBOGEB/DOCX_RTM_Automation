    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if (
                brace_count == 0
                and current_block
                and any("{" in l for l in current_block)
            ):
        ...
        if valid_blocks:
            ...
            try:
                ...
                print("   ✅ SUCCESSFULLY FIXED using block strategy!")
                print(f"   📏 New size: {len(fixed_content)} characters")
                return True

            except json.JSONDecodeError:
                print("   ❌ Fixed content is still invalid")

        # Strategy 3: Create minimal VS Code settings
        print("   🔧 Creating minimal VS Code settings...")
...
def aggressive_fix_launch_json(file_path):
    ...
    try:
        ...
        fixed_content = re.sub(
            r"(\s+)(name|type|request|program|console|args|stopOnEntry|cwd)\s*:",
            r'\1"\2":',
            fixed_content,
        )
        ...
        try:
            ...
            print("   ✅ SUCCESSFULLY FIXED launch.json!")
            return True
            ...
            print("   🔧 Creating minimal launch.json...")
            ...
            print("   ✅ Created minimal launch.json!")
            return True
...
def inspect_problematic_file(file_path):
    ...
    try:
        ...
        if len(lines) >= 25:
            print("   🎯 Around line 25 (error location):")
            for i in range(22, min(28, len(lines))):
                line_num = i + 1
                line = lines[i]
                print(
                    f"      {line_num:2d}: {line[:80]}{'...' if len(line) > 80 else ''}"
                )
...
def main():
    ...
    if fixed_count == 4:
        print("\n🎉 ALL FILES FIXED!")
        print("🚀 Your RTM system now has 100% JSON health!")
        print("\n💡 Verify the fixes:")
        print("   python json_file_analyzer_safe.py")
    else:
        print("\n⚠️ Some files may need manual editing")
        print("💡 Check the backup files and VS Code for valid JSON")
