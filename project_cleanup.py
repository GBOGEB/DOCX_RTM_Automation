...
class AsciiDiagramGenerator:
    def __init__(self, indent_size: int = 2, max_width: int = 80, indent_chars: str = "  ", branch_chars: Dict[str, str] = None):
        ...
        if 'sections' in outline_data:
            ...
            for i, section in enumerate(sections):
                ...
                for l in range(1, level):
                    ...
                    if l < level - 1 and last_level[l] > 0:
                        indent += self.branch_chars['vertical'] + " " * (self.indent_size - 1)
                    else:
                ...
                if level == 1:
                    result.append(f"{connector}{self.branch_chars['horizontal']} {section_text}")
                else:
                    result.append(f"{indent}{connector}{self.branch_chars['horizontal']} {section_text}")
            ...
            for key, value in outline_data.items():
                if isinstance(value, list) and len(value) > 0 and isinstance(value[0], dict):
                    ...
                    for item in value:
                        if 'title' in item:
                            result.append(f"{self.branch_chars['bullet']} {item['title']}")
                        elif 'name' in item:
                            result.append(f"{self.branch_chars['bullet']} {item['name']}")
    ...
    def _process_headers(self, headers: List[Dict[str, Any]], result: List[str], depth: int = 0) -> None:
        ...
        for header in headers:
            ...
            if depth > 0:
                connector = f"{self.branch_chars['tee']} " if depth == 1 else f"{self.branch_chars['vertical']}   " * (depth-1) + f"{self.branch_chars['tee']} "
            else:
                connector = ""
        ...
        for req in requirements:
            ...
            if links:
                for link in links:
                    ...
                    result.append(f"  {self.branch_chars['corner']}{self.branch_chars['horizontal']}{'─' if self.branch_chars['horizontal'] == '-' else self.branch_chars['horizontal']} {relation} → [{target}]")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate ASCII diagrams from document structure')
    parser.add_argument('input_file', help='Input YAML or JSON file containing outline or RTM data')
    parser.add_argument('--type', choices=['outline', 'rtm'], default='outline', 
