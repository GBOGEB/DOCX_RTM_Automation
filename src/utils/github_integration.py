...
class GitHubIntegration:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.commit_message = self.github_config.get("commit_message", "Update from RTM automation")
    ...
    def clone_repository(self) -> bool:
        ...
        if local_dir.exists() and (local_dir / ".git").exists():
            logger.info(f"Repository already exists at {self.local_path}, pulling latest changes instead")
            return self.pull_repository()
        
        # Create parent directory if needed
        os.makedirs(os.path.dirname(self.local_path) if os.path.dirname(self.local_path) else ".", exist_ok=True)
    ...
    def pull_repository(self) -> bool:
        ...
        if not local_dir.exists() or not (local_dir / ".git").exists():
            logger.warning(f"{self.local_path} is not a git repository, cloning instead")
    ...
    def commit_and_push_changes(self, files: List[str], message: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if self.user_name:
                subprocess.run(["git", "config", "user.name", self.user_name], check=True)
            if self.user_email:
                subprocess.run(["git", "config", "user.email", self.user_email], check=True)
...
def debug_github_integration(config: Dict[str, Any]) -> int:
    ...
    if not github.enabled:
        ...
        print("To enable, set 'enabled: true' in the github section of your configuration.")
        return 0
