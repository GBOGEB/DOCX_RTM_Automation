...
class DependencyManager:
    ...
    def check_pandoc_modules(self) -> bool:
        ...
        for module_name, module_path in pandoc_modules.items():
            ...
            if not path.exists():
                self.warnings.append(f"Pandoc module '{module_name}' not found at {path}")
                modules_ok = False
            else:
        ...
        if not pandoc_integration_path.exists():
            self.warnings.append(f"Pandoc integration module not found: {pandoc_integration_path}")
        else:
    ...
    def check_required_packages(self, packages: List[str]) -> bool:
        ...
        if missing_packages:
            ...
            self.warnings.append(f"To install missing packages, run: pip install {packages_str}")
            return False
    ...
    def validate_all(self) -> Tuple[bool, List[str], List[str]]:
        ...
        if any(key in self.config for key in ["pandoc_options", "pandoc_modules", "pandoc_path"]):
...
if __name__ == "__main__":
    ...
    test_config = {
        "pandoc_options": {
            "lua_filter": "config/filters/rtm_filter.lua"
        },
        "pandoc_modules": {
