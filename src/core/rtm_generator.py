...
class RTMGenerator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.config.get("requirement_patterns"):
            self.logger.debug(f"Loaded {len(self.config['requirement_patterns'])} requirement patterns.")
        ...
        if paths_config:
            ...
            self.logger.debug(f"Paths config: input_dir='{input_dir}', output_dir='{output_dir}', logs_dir='{logs_dir}', config_dir='{config_dir}'")

        if self.config.get("external_md_input"):
            self.logger.debug(f"External MD input path found: {self.config['external_md_input']}")
        
        if self.config.get("rtm_yaml"):
             self.logger.debug(f"RTM YAML output path configured: {self.config['rtm_yaml']}")

        pandoc_opts = self.config.get("pandoc_options")
        if pandoc_opts:
            self.logger.debug(f"Pandoc options found: TOC enabled: {pandoc_opts.get('toc', 'N/A')}, Lua filter: {pandoc_opts.get('lua_filter', 'N/A')}")

        github_config = self.config.get("github")
        if github_config:
            self.logger.debug(f"GitHub configuration found: Enabled: {github_config.get('enabled', 'N/A')}, Repo URL: {github_config.get('repo_url', 'N/A')}")
        ...
        if isinstance(pipeline_steps_config, list):
            self.logger.debug(f"Pipeline steps configuration found with {len(pipeline_steps_config)} steps.")
        elif pipeline_steps_config: # If it exists but is not a list (e.g. misconfigured)
        ...
        if pandoc_modules_config and isinstance(pandoc_modules_config, dict):
            ...
            self.logger.warning("Pandoc modules configuration found but is not a dictionary.")

        requirements_list_config = self.config.get("requirements")
        if isinstance(requirements_list_config, list):
            self.logger.debug(f"Requirements list configuration found with {len(requirements_list_config)} items.")
        elif requirements_list_config:
            self.logger.warning("Requirements list configuration found but is not a list.")

        sections_list_config = self.config.get("sections")
        if isinstance(sections_list_config, list):
            self.logger.debug(f"Sections list configuration found with {len(sections_list_config)} items.")
        ...
        doc_metadata_config = self.config.get("metadata") # This is the document metadata from paths.yaml
        if doc_metadata_config and isinstance(doc_metadata_config, dict):
            ...
            self.logger.warning("Document metadata configuration found but is not a dictionary.")
    ...
    def generate_rtm(self, input_files: List[str]) -> Dict[str, Any]:
        ...
        project_version = self.config.get("project", {}).get("version", "0.1.0-core-placeholder")
        ...
        if sections_data:
            ...
            for section in sections_data:
                if isinstance(section, dict):
                    clean_section = {k: v for k, v in section.items()}  # Copy all properties
            ...
            self.logger.info(f"Including {len(clean_sections)} sections in outline data")
                ...
                for i, line in enumerate(lines):
                    ...
                    if stripped_line.startswith('#'):
                        ...
                        if heading_text in section_map:
                            ...
                            current_section_number = section_map[current_section]["number"]
                            self.logger.debug(f"Found section: {current_section} - {current_section_number}")
                        # Check for section number match
                        else:
                            # Try to extract section number at the start of heading
                            section_match = re.match(r'^\s*(\d+(\.\d+)*)\s+(.+)$', heading_text)
                            if section_match:
                                ...
                                if extracted_number in section_number_map:
                                    current_section = section_number_map[extracted_number]
                                    current_section_number = extracted_number
                                    self.logger.debug(f"Found section by number: {current_section} - {current_section_number}")
                    ...
                    if is_requirement:
                        req_text = line.strip()
                        self.logger.debug(f"Found requirement in {md_file_path.name}, line {i+1}: {req_text[:50]}...")
                        ...
                        if current_section:
                            ...
                            requirement_entry["section_level"] = section_map[current_section]["level"]
        ...
        self.logger.info(f"CoreRTMGenerator found {len(rtm_data['requirements'])} requirements.")
        return rtm_data
...
if __name__ == '__main__':
    ...
    mock_config = {
        ...
        "location_hooks": {
            "section": {"enabled": True, "prefix": "LH1S_", "use_section_titles": False},
            "lcp_phase": {"enabled": True, "prefix": "LH2P_", "default_phase": "2"}
        },
        "attributes": [
            {"name": "priority", "values": ["high", "medium", "low"], "default": "high"},
            {"name": "status", "values": ["proposed", "approved"], "default": "proposed"},
        ],
