...
class DocumentStructure:
    ...
    def __init__(self):
        ...
        self.dependencies = []  # List of (source_id, target_id, rel_type) tuples
    ...
    def parse_markdown(self, markdown_file: str) -> bool:
        ...
        try:
            ...
            for match in re.finditer(heading_pattern, content, re.MULTILINE):
                ...
                logger.debug(f"Found heading: L{level} {section_num} {title} [LCP:{lcp_phase}]")
            ...
            for match in re.finditer(req_div_pattern, content, re.DOTALL):
                ...
                req_id = req_id_match.group(1) if req_id_match else f"REQ-AUTO-{len(self.requirements)+1}"
            ...
            for pattern in para_req_patterns:
                for match in re.finditer(pattern, content, re.DOTALL):
                    ...
                    if any(req["id"].upper() == req_id.upper() for req in self.requirements):
                        continue
                    ...
                    logger.debug(f"Found paragraph requirement: {req_id} in section {section}")
            ...
            for pattern in dep_patterns:
                for match in re.finditer(pattern, content, re.IGNORECASE):
                    ...
                    logger.debug(f"Found dependency: {source_id} {rel_type} {target_id}")
            ...
            for match in re.finditer(table_pattern, content):
                ...
                for target_id in targets:
                    ...
                    logger.debug(f"Found table dependency: {source_id} depends_on {target_id}")
    ...
    def _find_section_for_position(self, position: int, content: str) -> str:
        ...
        heading_matches = list(re.finditer(heading_pattern, content[:position], re.MULTILINE))
    ...
    def generate_structure_tree(self) -> str:
        ...
        for i, (level, number, title, lcp_phase) in enumerate(self.headings[1:], 1):
            ...
            for next_heading in self.headings[i+1:]:
                if next_heading[0] <= level:  # If we find another heading at same or higher level
                    is_last = (next_heading[0] < level)  # It's last only if the next is at a higher level
                    break
            ...
            if level == 1:
                ...
                branch_chars[level] = "│   "  # Continue vertical line for this level
    ...
    def generate_dependency_graph(self) -> str:
        ...
        output = ["", "REQUIREMENTS DEPENDENCY GRAPH", "============================", ""]
        ...
        for source_id, targets in dep_groups.items():
            ...
            for req in self.requirements:
                if req["id"].upper() == source_id.upper():
                    source_desc = req["text"][:30] + ("..." if len(req["text"]) > 30 else "")
                    break
                ...
                if i == 0:
                    ...
                    for req in self.requirements:
                        if req["id"].upper() == target_id.upper():
                            target_desc = req["text"][:30] + ("..." if len(req["text"]) > 30 else "")
                            break
...
def extract_structure_from_md(input_file: str, output_file: str) -> bool:
    ...
    if os.path.exists(raw_json):
        ...
        if not doc_structure.process_raw_structure_json(raw_json):
            logger.warning("Failed to process raw structure data, falling back to markdown parsing")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate document structure visualization from markdown")
    parser.add_argument("input", help="Input markdown file")
    parser.add_argument("--output", "-o", help="Output file", default="output/document_structure.txt")
