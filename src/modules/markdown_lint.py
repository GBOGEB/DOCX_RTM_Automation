...
class MarkdownLinter:
    ...
    def lint(self, markdown_content: str) -> List[Dict[str, Any]]:
        ...
        if not markdown_content:
            return [{'rule': 'empty_content', 'message': 'Markdown content is empty', 'line': 0}]
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith('#'):
                ...
                if '#' in line[1:].strip():
                    ...
                    if closing_match and len(closing_match.group(1)) != len(closing_match.group(3)):
                        issues.append({
                            'rule': 'heading_format',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Lint Markdown files for documentation quality')
    parser.add_argument('files', nargs='+', help='Markdown files to lint')
    parser.add_argument('--json', action='store_true', help='Output in JSON format')
    parser.add_argument('--fix', action='store_true', help='Attempt to fix issues (not all can be fixed)')
    ...
    for file_path in args.files:
        try:
            ...
            if args.json:
                all_issues[file_path] = [{'rule': 'file_error', 'message': str(e), 'line': 0}]
            else:
