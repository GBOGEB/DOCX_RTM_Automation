...
def generate_mindmap_diagram(outline: Dict[str, Any], max_depth: int = 7) -> str:
    ...
    def generate_branch(level, parent_index, indent=""):
        ...
        parent_number = "" if level == 1 else sections_by_level[level-1][parent_index].get('number', '')
        filtered_sections = [s for s in sections_by_level[level] 
...
def main() -> int:
    """Main function"""
    parser = argparse.ArgumentParser(description='Generate ASCII diagram from document outline')
    parser.add_argument('input', help='Input YAML or JSON file containing document outline')
    parser.add_argument('-o', '--output', help='Output file for the ASCII diagram (default: stdout)')
    parser.add_argument('-d', '--depth', type=int, default=7, help='Maximum depth to display (default: 7)')
    parser.add_argument('-t', '--type', choices=['basic', 'tree', 'mindmap'], default='tree',
    ...
    try:
        ...
        # Output
        if args.output:
            # Fix: Write with UTF-8 encoding explicitly to avoid encoding errors
