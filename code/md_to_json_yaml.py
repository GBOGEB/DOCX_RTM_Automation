...
def main():
    ...
    md_output_key = paths.get("md_output", "output/default_master.md") # Default if not in config
    ...
    data = {
        "requirements": [l.strip() for l in lines if re.match(r"^QQQ_[0-9]{3}", l)], # Assuming QQQ_ is a requirement prefix
        "headings": [l.strip() for l in lines if re.match(r"^#{1,7}\s", l)],
    }

    yaml_output_key = paths.get("yaml_output", "output/parsed_md_data.yaml") # Default if not in config
    json_output_key = paths.get("json_output", "output/parsed_md_data.json") # Default if not in config
