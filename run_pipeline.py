...
def check_prerequisites():
    ...
    if python_version.major < 3 or (python_version.major == 3 and python_version.minor < 8):
        logger.error(f"Python 3.8 or higher required. Current version: {python_version.major}.{python_version.minor}")
        return False
    
    logger.info(f"Python version check passed: {python_version.major}.{python_version.minor}")
...
def run_pipeline():
    ...
    if config.get('github', {}).get('enabled', False):
        if not setup_git_environment(config):
            logger.warning("Failed to setup Git environment. Continuing with pipeline...")
    ...
    for step in pipeline_steps:
        if step.get('enabled', False):
            ...
            try:
                ...
                logger.info(f"Step '{name}' completed successfully in {step_duration:.2f} seconds.")
            except subprocess.CalledProcessError as e:
                step_duration = time.time() - step_start_time
                logger.error(f"Error running step '{name}' after {step_duration:.2f} seconds: {e}")
    ...
    if config.get('github', {}).get('enabled', False) and all_steps_success:
        commit_message = config.get('github', {}).get('commit_message', 
                                                    'Automatic update from pipeline')
        commit_changes(config, commit_message)
    
    total_duration = time.time() - start_time
    if all_steps_success:
        logger.info(f"Pipeline execution completed successfully in {total_duration:.2f} seconds!")
    else:
        logger.warning(f"Pipeline execution completed with errors in {total_duration:.2f} seconds.")
