...
def test_v031_registers_reliability_and_mission_control_without_authority_transfer():
...
def test_reliability_is_crosscut_profile_not_primary_lane():
    app = load_applicability()
    lanes = {lane["lane_id"]: lane for lane in app["triage_lanes"]}
    assert set(lanes) == {"TRIAGE-QPS", "TRIAGE-ADR", "TRIAGE-OCD", "TRIAGE-RTM-DTM"}
    assert "TRIAGE-RELIABILITY" not in lanes

    profile = app["reliability_analysis_profile"]
    assert profile["topic_is_not_lane"] is True
    assert profile["role"] == "ANALYTICAL_CONSUMER_PROFILE"
    assert profile["lane_mapping"] == {
        "contractual_requirement": "TRIAGE-QPS",
        "architecture_redundancy_common_cause": "TRIAGE-ADR",
        "degraded_recovery_maintenance_state": "TRIAGE-OCD",
        "source_evidence_traceability": "TRIAGE-RTM-DTM",
    }
    assert "reliability_source_atom" in profile["accepted_inputs"]
    assert "reliability_model_candidate" in profile["outputs"]
    assert "Component MTBF" in profile["guard"]
    assert "Table-10" in profile["guard"]
...
def test_control_gates_fail_closed_on_authority_method_and_reliability_lane():
    app = load_applicability()
    ...
    assert "TRIAGE-RELIABILITY is forbidden" in gates["QPS-TRIAGE-GATE-012"]["rule"]


def test_lane_assignment_gate_names_only_canonical_four_primary_lanes():
    app = load_applicability()
    gates = {gate["gate_id"]: gate for gate in app["control_gates"]}
    rule = gates["QPS-TRIAGE-GATE-002"]["rule"]
    for lane in ["TRIAGE-QPS", "TRIAGE-ADR", "TRIAGE-OCD", "TRIAGE-RTM-DTM"]:
        assert lane in rule
    assert "TRIAGE-RELIABILITY" not in rule
